<!-- Caso o seja a vizualização estilizada -->

<!-- src/template/adiministradores/home_adm_estilizada.php -->

<!-- Dentro do Conteúdo do site -->
<h1>estilizada</h1>
<div class="formulario">
    <div class="form">
        <div class="form-group link-botao">
            <br>
            <div class="links-auxiliares">
                <a class="link-auxiliar c1" href="<?php echo TEMPLATE_URL; ?>administradores/cadastro_u_adm.php" target="_blank">Cadastar um Usuario</a>
                <div class="gambiarra"></div>
                <a class="link-auxiliar c2" href="<?php echo TEMPLATE_URL; ?>administradores/listar_u_adm.php" target="_blank">Editar um Usuario</a>
            </div>
            <br>
            <div class="links-auxiliares">
                <a class="link-auxiliar c3" href="<?php echo TEMPLATE_URL; ?>administradores/cadastro_v_adm.php" target="_blank">Cadastar um Vendedor</a>
                <div class="gambiarra"></div>
                <a class="link-auxiliar c4" href="<?php echo TEMPLATE_URL; ?>administradores/listar_v_adm.php" target="_blank">Editar um Vendedor</a>
            </div>
            <br>
        </div>
    </div>
</div>
